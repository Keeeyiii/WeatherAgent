...
### 8.3 基线对比实验（与随机森林使用同一滚动窗口划分）

在随机森林之外，另设两种简单基线方法（与随机森林使用**完全相同的有效样本与滚动窗口划分**，保证四种方法在同一测试集上可比）：

- **基线A（常数去偏）**：`corrected = forecast - 训练集平均偏差`
- **基线B（按小时去偏）**：按 24 个小时，分别用训练集中该小时的平均偏差订正

四种方法在各折测试集上的平均表现：

| 方法 | MAE（均值 ± 标准差） | RMSE（均值） | Bias（均值） |
|------|---------------------|-------------|-------------|
| 原始预报 | 1.3827 ± 0.0748 ℃ | 1.7278 ℃ | +1.2077 ℃ |
| 基线A（常数去偏） | 1.0544 ± 0.0414 ℃ | 1.2364 ℃ | -0.0262 ℃ |
| 基线B（按小时去偏） | 0.5368 ± 0.0815 ℃ | 0.6619 ℃ | -0.0262 ℃ |
| Random Forest | 0.5586 ± 0.0694 ℃ | 0.6845 ℃ | -0.0723 ℃ |

结果要点：

- 基线A 仅消除整体系统性偏差，即已使 MAE 下降 23.7%；基线B 进一步消除偏差的日变化分量，MAE 相对原始预报下降 61.2%（降至 0.5368 ℃）。
- **随机森林并未超过基线B**：其 MAE（0.5586 ℃）比基线B 反而高出 4.1%。
- 这说明在本示例数据上，随机森林的订正收益主要来自**系统性偏差和日变化偏差的消除**，而非更复杂的非线性关系；简单的按小时去偏已能抓住绝大部分可订正信号。
...
- 基线对比（8.3 节）进一步表明：随机森林的收益主要来自系统性偏差与日变化偏差的消除，按小时去偏的简单基线已达到相近甚至略好的订正效果。
