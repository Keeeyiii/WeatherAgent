...
class WeatherAgent:
    ...
    def _tool_ml_correction(self):
        ...
        folds = [
            {
                ...
                "corrected_mae": round(f["corrected"]["MAE"], 4),
                "corrected_rmse": round(f["corrected"]["RMSE"], 4),
                "corrected_bias": round(f["corrected"]["Bias"], 4),
        ...
        return {
            "method": "rolling_window_cv（滚动窗口交叉验证，训练窗口逐折扩大）",
            "n_splits": result["n_splits"],
            "folds": folds,
            "raw_mean": {k: round(v, 4) for k, v in result["raw_mean"].items()},
            "raw_std": {k: round(v, 4) for k, v in result["raw_std"].items()},
            "corrected_mean": {
                k: round(v, 4) for k, v in result["corrected_mean"].items()
            },
            "corrected_std": {
                k: round(v, 4) for k, v in result["corrected_std"].items()
            },
            ...
            "feature_importance_avg": {
                k: round(float(v), 4) for k, v in result["importance"].items()
            },
            "note": (
                "corrected_mean / corrected_std 为各折订正后指标的均值与标准差，"
                "标准差越小表示订正效果在不同时间段越稳定。"
            ),
        }
