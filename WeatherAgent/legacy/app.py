...
if st.session_state.get("ml_done", False):
    ...
    st.markdown(
        ...
        **最优插值（Optimal Interpolation, OI）是资料同化中最经典的分析方法**：
        给定背景场（background / first guess，这里取模式预报值）与观测，
        按两者误差方差之比分配权重，使分析值（analysis，融合观测与背景场后
        的最优估计）的误差方差最小。OI 在 20 世纪 80—90 年代曾是全球业务
        数值预报的主流同化方案，后逐步被 3D-Var 取代，但其「按误差统计
        最优加权」的思想仍是现代资料同化方法的基础。
        ...
        - OI 权重 `w = sigma_b² / (sigma_b² + sigma_o²)`
          （`sigma_o² → 0` 时 `w → 1`，完全采信观测新息；
          `sigma_b² → 0` 时 `w → 0`，预报本身已很准则几乎不订正）
        - 虚拟观测新息 = 训练集上 `observed − forecast` 的均值
          （新息 innovation = 观测 − 背景场，等于项目 Bias 取反；
          为负表示预报系统性偏高）
...
st.markdown(
    ...
    **分工：**
    - Python 负责：数据读取、质量检查、MAE/RMSE/Bias 计算、随机森林订正、简化最优插值（OI）实验等真实计算。
